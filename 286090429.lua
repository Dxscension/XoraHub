...
print("Fragile library is loading. [3/5]")
local dragSpeed = 0
...
-- Defining libraries
library.TITLE = library.TITLE or "Arsenal"
...
function library:CreateWindow(Text,IsPremium,Icon)
	...
	TemplateSidebarButton.MouseButton1Click:Connect(function()
		local ANIm_1 = Instance.new("Frame")

		ANIm_1.Name = "ANIm_1"
		ANIm_1.Parent = MAINGUI
		ANIm_1.BackgroundColor3 = Color3.fromRGB(13, 13, 13)
		ANIm_1.BackgroundTransparency = 1.000
		ANIm_1.BorderSizePixel = 0
		ANIm_1.Position = UDim2.new(0.278350502, 0, 0.0839328915, 0)
		ANIm_1.Size = UDim2.new(0, 353, 0, 383)
		ANIm_1.ZIndex = 10000
		TweenService:Create(ANIm_1 , TweenInfo.new(0.26, Enum.EasingStyle.Quad , Enum.EasingDirection.InOut), {BackgroundTransparency = 0}):Play()	
		...
		TweenService:Create(ANIm_1 , TweenInfo.new(0.26, Enum.EasingStyle.Quad , Enum.EasingDirection.InOut), {BackgroundTransparency =1}):Play()	
		...
		function H:CreateSection(Text, side)
			...
			function Intern:CreateColourWheel(Text,Frame,type ,Callback)
                ...
                elseif type == "Color" then
                    SmallyDisplay.BackgroundColor3=Frame.Color
                    def=SmallyDisplay.BackgroundColor3
				end
				...
				Pikerbuton.MouseButton1Click:Connect(function()
					...
					ColourDisplay.ImageColor3 = Frame.BackgroundColor3 or Frame.TextColor3 or Frame.Color
					wait(.4)
					ANIM.Visible = true
					ANIM:TweenPosition(UDim2.new(-0.004,0,-0.052,0), Enum.EasingDirection.Out, "Quad", 0.3 )
			...
			function Intern:CreateLabel(Text,TextColor)
				Text=Text or "Not Defined"
				...
				Label.BackgroundColor3 = Color3.fromRGB(12, 12, 11)
				Label.BorderSizePixel = 0
				Label.ClipsDescendants = true
				Label.Position = UDim2.new(0.583453536, 0, 1.33407128, 0)
				Label.Size = UDim2.new(0, 145, 0, 20)
...
print("Fragile library is loaded, now loading variables. [4/5]")
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if tostring(NamecallMethod) == "FireServer" and tostring(self) == "ControlTurn" then
        if Client.Toggles.AntiAim == true then
            if Client.Values.LookMeth == "A" then --Look up
                Args[1] = 1.3962564026167
            end
            if Client.Values.LookMeth == "B" then -- Look down
                Args[1] = -1.5962564026167
            end
            if Client.Values.LookMeth == "C" then -- Smell ASS
                Args[1] = -8.1
            end
            if Client.Values.LookMeth == "D" then -- Top idk
                Args[1] = -3.1 --3.1
            end
            if Client.Values.LookMeth == "E" then -- Legs in torso
                Args[1] = -6.1;
            end
            return namecallold(self, unpack(Args))
        end
...
function CK()
    if Client.Values.LookMeth == "A" then
        Client.Values.LookMeth="B"
    elseif Client.Values.LookMeth == "B" then
        Client.Values.LookMeth="C"
    elseif Client.Values.LookMeth== "C" then
        Client.Values.LookMeth ="D"
    elseif Client.Values.LookMeth =="D" then
        Client.Values.LookMeth = "E"
    elseif Client.Values.LookMeth == "E" then
        Client.Values.LookMeth="A"
    end
end
...
local w1=library:CreateWindow("Aiming",false, library.icons.Gun)
local w2=library:CreateWindow("Local",false, library.icons.Gun)

local sa=w1:CreateSection("Silent Aim", "Left")
local S = w1:CreateSection("FOV Options", "Right")
local sd=w2:CreateSection("LocalPlayer", "Right")
local s1=w2:CreateSection("Gun Mods", "Left")
...
S:CreateToggle('Using FOV', function(value)
    Client.Toggles.FOV = value
    FOVCircle.Visible = value
end)

S:CreateColourWheel("FOV Color", FOV ,"Color", function(v)
	if v[1] == 0 then
		FOV.Color = v[2]
	else
		FOV.Color = Color3.fromRGB(v[2])
	end
end)


S:CreateSlider('FOV Radius', 2, 500, {false,0,460}, function(value)
    Client.Values.FOV = value
end)
S:CreateSlider('FOV Thickness', 1, 5, {false,0,2}, function(value)
    FOVCircle.Thickness  = value
end)
S:CreateSlider('FOV NumSides', 3, 80, {false,0,75}, function(value)
    FOVCircle.NumSides = value
end)

sd:CreateSlider('Walkspeed', 23, 200, {false,0,23}, function(value)
    game.Players.LocalPlayer.Character:WaitForChild("Humanoid").WalkSpeed = value
end)
sd:CreateToggle('Inf Jump', function(v)
    Client.Toggles.InfJump = v
end)

sd:CreateToggle('Third Person', function(state)
    if state then 
...
--[[sd:CreateToggle('Anti Aim', function(v)
    Client.Toggles.AntiAim = v
end)
]]
sd:CreateToggle('Fast Heal', function(v)
...
sd:CreateButton('Fly (X)', function(v)
    repeat wait() 
...
sa:CreateToggle('Silent Aim',  function(state)
    Client.Toggles.SilentAim = state
end)

sa:CreateToggle('Semi-WallBang',  function(state)
    Client.Toggles.WallBang = state
end)

sa:CreateDropDown('Aim Part', {'Head','LowerTorso','Random'} , nil, function(s)
    Client.Values.AimPart = s
end)


--[[sa:CreateToggle('Draw FOV',  function(state)
    Client.Toggles.FOV = state
end)
]]--

s1:CreateToggle('Infinite Ammo', function(state)
    Config.Infammo = state
    game:GetService("RunService").Stepped:connect(function()
        task.spawn(function()
        if Config.Infammo then
                getsenv(game:GetService("Players").LocalPlayer.PlayerGui.GUI.Client.Functions.Weapons).ammocount.Value = 30
                getsenv(game:GetService("Players").LocalPlayer.PlayerGui.GUI.Client.Functions.Weapons).ammocount.Value = 31
            end
        end)
    end)
end)
s1:CreateToggle('Always Auto', function(state)
...
s1:CreateToggle('Fast Firerate', function(state)
...
s1:CreateToggle('No Recoil', function(state)
...
s1:CreateToggle("No Spread", function(state)
...
print("Fragile has been inyected Correctly. [5/5]")
