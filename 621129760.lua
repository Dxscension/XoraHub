...
-- Defining libraries
library.TITLE = library.TITLE or "Knife Ability Test | KAT"
...
local NoCool = settings.NoCooldown

-- IMPORTNANT FUNCTIONS --
function getClosestPlayerToCursor(x, y)
...
function NoCooldown(v)
	...
	if knife == nil then
		knife = game.Players.LocalPlayer.Character:FindFirstChild("Knife")
		if knife == nil then
			knife = game.Players.LocalPlayer.Backpack:FindFirstChild("Knife")
		end
		scr = getsenv(knife.KnifeServer.KnifeClient)
		KnifeClient = knife
	else
		scr = getsenv(knife.KnifeServer.KnifeClient)
		KnifeClient = knife
	...
	if KnifeClient  and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character:FindFirstChild("Head") and not v.Character:FindFirstChild("ForceField") then
		local KnifeClientS = (scr)
...
mt.__index = newClose(function(t, k)
	if settings.SilentAim ==true then
		if not checkcaller() and t == mouse and tostring(k) == "X" and string.find(getfenv(2).script.Name, "Client") and getClosestPlayerToCursor() and settings.SilentAim == true then
			local closest = getClosestPlayerToCursor(oldIndex(t, k), oldIndex(t, "Y")).Character[settings.AimPart]
			if true then
				local pos = currentCamera:WorldToScreenPoint(closest.Position)
				return pos.X
			end
		end
		if not checkcaller() and t == mouse and tostring(k) == "Y" and string.find(getfenv(2).script.Name, "Client") and getClosestPlayerToCursor() and settings.SilentAim == true then
			local closest = getClosestPlayerToCursor(oldIndex(t, "X"), oldIndex(t, k)).Character[settings.AimPart]
			if true  then
				local pos = currentCamera:WorldToScreenPoint(closest.Position)
				return pos.Y
			end
		end
		if t == mouse and tostring(k) == "Hit" and string.find(getfenv(2).script.Name, "Client") and getClosestPlayerToCursor() and settings.SilentAim == true then
			local closest = getClosestPlayerToCursor(oldIndex(t, "X"), oldIndex(t, k)).Character[settings.AimPart]
			if true  then
				return getClosestPlayerToCursor(mouse.X, mouse.Y).Character[settings.AimPart].CFrame
			end
		end
	end
...
RunService.RenderStepped:Connect(function()
    ...
    if settings.Autofarm == true then
			...
			for i, v in pairs(game.Players:GetChildren()) do
				if
					v ~= game.Players.LocalPlayer and v.Character ~= nil and
				 ...
				 then
					...
					if game.Players.LocalPlayer.Character:FindFirstChild("Knife") then
						v.Character:FindFirstChild("HumanoidRootPart").CFrame =
							...
							game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame.LookVector * 2
						wait(0.1)
						game:GetService "VirtualUser":Button1Down(Vector2.new(1, 1))
						wait(0.1)
						game:GetService "VirtualUser":Button1Up(Vector2.new(1, 1))
					elseif
						game.Players.LocalPlayer.Backpack:FindFirstChild("Knife") and
							not game.Players.LocalPlayer.Character:FindFirstChild("Knife")
					 then
...
library.TITLE = "Knife Ability Test"
library:CreateSidebarLabel("Main")

local w1=library:CreateWindow("Aiming",false, library.icons.Gun)

local s1=w1:CreateSection("Silent Aim", "Left")
local s2 = w1:CreateSection("Miscellaneous", "Right")

local T3 = s1:CreateToggle("Silent Aim", function(v)
    settings.SilentAim = v
end)

s1:CreateDropDown("Aim Part", {"Head","Torso"}, nil, function(v)
	settings.AimPart=v
end)


local T2=s2:CreateButton("No Cooldown", function()
	NoCooldown(localPlayer)
end)

s2:CreateToggle("GodMode", function(v)
	if v == true then
		if not game.Players.LocalPlayer.Character:FindFirstChild( "Kevin loves nigga penis" ) then
			local plr = game.Players.LocalPlayer
			local chr = plr.Character:FindFirstChild("HumanoidRootPart")
			chr.Parent = nil
			chr:Clone()
			chr.Parent = plr.Character
			chr.Name = "Kevin loves nigga penis."
		end
	else
		local plr = game.Players.LocalPlayer
		local chr = plr.Character:FindFirstChild("Head")
		chr:Destroy()
	end
end)
s2:CreateToggle("Autofarm", function(v)
	settings.Autofarm = v
end)


s2:CreateToggle("Best Settings", function(v)
	T2:Press(v)
	T3:UpdateValue(v)
end)


--[[local w2=library:CreateWindow("Murder",false, 129697950) --8217650131

local S1=w1:CreateSection("Sherif & Murder", "Left")
S1:CreateLabel("Soon...")
